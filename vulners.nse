...
local diretory = stdnse.get_script_args("vulners.diretory")
local metrics = stdnse.get_script_args("vulners.metrics")
if diretory then
  diretory = string.format(diretory)
end
if metrics then
  metrics = string.format(metrics)
end
...
-- Trecho novo  
  if diretory and #output > 0 and metrics then 
    local jsonString = cjson.encode(output)
    diretory = "python3 "..diretory
    diretory = diretory.." "
    diretory = diretory.."'%s'"
    local command_python = string.format(diretory, jsonString)
    if metrics then
      metrics = " '"..metrics
      metrics = metrics .."'"
      command_python = command_python..metrics
    end
-- Executa o comando Python a partir do Lua
    local handle  = io.popen(command_python)
    local output2 
    print("Communicating with the plugin...")
    if handle then
      handle:write(jsonString.. "\n")
      handle:flush()
      output2 = handle:read("*a")
      handle:close()
      output = {}
      -- Converte a string JSON de volta para uma tabela Lua
      output2 = cjson.decode(output2)
    else
      if #output > 0 then
    ...
    --Tempo de execução do plugin
    for _, value in ipairs(output2) do
--       print("Time of execution:", value.time , "segundos")
       print("Process Usage:", value.process_usage , "%")
       print("Memory Usage:", value.usage_memory , "MB") 
       print("Total number of vulnerabilities:", value.number_vuln)
       break
    end
    for key, value in ipairs(output2) do
       local v = {
        id = value.id,
        type = value.type,
      -- Mark the exploits out
        is_exploit = value.is_exploit,
      -- Sometimes it might happen, so check the score availability
        cvss = tonumber(value.cvss)
      }
      setmetatable(v, cve_meta)
      output[#output+1] = v	
    end	
